...
#parse xml tagged with #title string
  ...
  if ($categoryFILTER eq 'used') {
     ...
     @imdbOutput = qx{php "$path/imdb_all.php"   "$SearchImdbForTitle;"};
     s/\s+\z// for @imdbOutput;
     
     $imdbRATING = $imdbOutput[0];
     $imdbAGE = $imdbOutput[1];
     $imdbPOSTER = $imdbOutput[2];
     $imdbCOUNTRY = $imdbOutput[3];
     $imdbYEAR = $imdbOutput[4];     
     $imdbURL = $imdbOutput[5]; 
  }
  else
  {
	print STDERR color("red"), 'No match for', color("reset"); print STDERR " $SearchImdbForTitle\n";
	next;
  }
